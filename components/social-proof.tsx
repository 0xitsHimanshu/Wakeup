...
export function SocialProof() {
  return (
    <div className="w-full border-b border-[rgba(55,50,47,0.12)] dark:border-[rgba(255,255,255,0.08)] flex flex-col justify-center items-center">
      ...
      {/* Logo Grid */}
      <div className="w-full max-w-7xl mx-auto px-6 sm:px-8 md:px-12 lg:px-16 border-[rgba(55,50,47,0.12)] dark:border-[rgba(255,255,255,0.08)] flex justify-center items-start">
        <div className="w-full grid grid-cols-2 sm:grid-cols-4 md:grid-cols-4 gap-0 border border-[rgba(55,50,47,0.12)] dark:border-[rgba(255,255,255,0.08)] rounded-lg overflow-hidden">
          {Array.from({ length: 8 }).map((_, index) => {
            ...
            return (
              <div
                ...
                `}
              >
                <div className="w-6 h-6 xs:w-7 xs:h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 lg:w-10 lg:h-10 relative shadow-[0px_-4px_8px_rgba(255,255,255,0.64)_inset] dark:shadow-[0px_-4px_8px_rgba(0,0,0,0.3)_inset] overflow-hidden rounded-full bg-white dark:bg-[#2D2A27]">
                  <img src="/horizon-icon.svg" alt="Horizon" className="w-full h-full object-contain dark:invert dark:opacity-80" />
                </div>
                <div className="text-center flex justify-center flex-col text-[#37322F] dark:text-[#E8E6E3] text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl font-medium leading-tight md:leading-9 font-sans">
                  Acute
                </div>
              </div>
            )
